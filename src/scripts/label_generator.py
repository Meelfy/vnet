...
class LabelGenerator(object):
    ...
    def split_passage(self,
                      passage: str):  # 'str' -> [str]
        return self._wordSplliter.split_words(passage)
    def gen_gold_span(self,
                      ...
                      ):  # 返回best_span=[start, end], best_span_roughl, passage_split, best_span_text 如果没有符合要求的 [], 0, passage_split, ''
        """
        ...
        :param extend_length: if len(answer) < 20 : candidate spans are extended to len(candidate span) + 2 * extend_length
                              else: extended to 3 * len(candidate span)
        :param roughl_threshold: best span must have a rough-L value that > roughl_threshould
        :return:  best_span=[start, end], best_span_roughl, passage_split, best_span_text 如果没有符合要求的 [], 0, passage_split, ''
        """
        # split passages and answers
        passage_split_spacy = self.split_passage(passage.lower())
        passage_split = []
        ...
        answer_idx = numpy.argmax(recall_answers)
        ...
        for i in range(candidate_length):
            ...
            best_span_, best_span_roughl_ = self.get_answers_with_RougeL(passage_split[start_idx_:(end_idx_ + 1)],
    ...
    def get_LCSubstr(passage, answer):  # 返回start_id[] end_id[] 按lcs长度排序 个数为有相同字符的子串
        ...
        record = numpy.array(record).reshape(-1)
...
# test code
a = 'ads fdsf sds dsd d, we %, ds daf kwef dddd'
b = [', dd ds daf kwef','fdsf sds d']
cc = LabelGenerator()
span, score, p, p2 = cc.gen_gold_span(a,b,roughl_threshold=0.7)
print(span)
print(score)
print(p)
print(p2)
